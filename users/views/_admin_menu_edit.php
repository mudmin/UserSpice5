...
<div class="content mt-3">
  <div class="d-flex justify-content-between align-items-center mb-3">
    <h2><?= !empty($item->label)? $item->label : ($itemId == 'new'? 'New Item' : $menu->menu_name) ?></h2>
  </div>
            ...
            <form method="post">
              ...
              <div class="form-group">
                ...
                <div class="form-check form-check-inline">
                  <input class="form-check-input" type="checkbox" id="permission_0" name="permissions[]" value="0" <?= in_array(0, $item->permissions)? 'checked': ""?>>
                  <label class="form-check-label" for="permission_0">Public</label>
                </div>
                <?php foreach($perms as $perm): ?>
                  <div class="form-check form-check-inline">
                    <input class="form-check-input other-perms" type="checkbox" id="permission_<?=$perm->id?>" name="permissions[]" value="<?=$perm->id?>" <?= in_array($perm->id, $item->permissions)? 'checked': ""?>>
              ...
              <div class="form-group" id="parent_wrapper">
                ...
                <select class="form-control" name="parent" id="parent" onchange="setDirty(true)">
                  <option value="<?=$item->parent?>"><?=parseMenuLabel($switchParent[$item->parent]); ?> (Current)</option>
                  <?php
                  foreach($switchParent as $k=>$v){
                  if($k == $item->parent){ continue; }
                  ?>
              ...
              <table class="table table-sm">
                ...
                ?>
                  <tr class="child-sortable" data-id="<?=$child->id?>">
                    ...
                    <td><?= $parsedLabel ?></td>
                    <td>
                      <?php if($parsedLabel != $child->label){ echo $child->label; } ?>
                    </td>
                    <td>
...
<script>
  ...
  new Sortable(childrenEl, {
    sortable: true, draggable: '.child-sortable',
    group: {name: "children", put: false, pull: false},
    store: {
      set: (sortable) => {
        ...
        $.ajax({
          ...
          type: 'POST',
          dataType: 'json',
          data: {action: 'sort_menu_items', order, menu_id: '<?=$menuId?>', item_id: '<?=$itemId?>', token: '<?=Token::generate()?>'}
        });
      }
    }
  });
