...
<?php if (!securePage(Server::get('PHP_SELF'))) {
  die();
} ?>
<?php
$errors = $successes = [];
...
if (strtotime($ctFormatted) < strtotime($dbPlus)) {
  Redirect::sanitized($actual_link);
}
if (!empty($_POST)) {
  $token = $_POST['csrf'];
  ...
  if (!empty($_POST['addPin'])) {
    ...
    if (empty($errors)) {
      ...
      if (!$db->error()) {
        ...
        $errors[] = 'There was an error updating your PIN. Please try again later.';
...
<div class="row">
  <form class="verify-admin" action="" method="POST" id="payment-form">
    ...
    <div class="col-md-4">
      ...
      <input type="hidden" name="verify_uri" value="<?= safeReturn($actual_link) ?>" />
      <input type="hidden" name="verify_page" value="<?= safeReturn($page) ?>" />
